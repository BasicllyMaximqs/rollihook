...
public:
	...
	constexpr CVector* GetRenderOrigin() noexcept
	{
		return memory::Call<CVector*>(this, 1);
	}
};
...
public: // netvars
	...
	NETVAR(accuracyPenalty, "CCSPlayer->m_fAccuracyPenalty", float)
	NETVAR(m_hActiveWeapon, "CCSPlayer->m_hActiveWeapon", std::int32_t)
	NETVAR(m_bHasDefuser, "CCSPlayer->m_bHasDefuser", bool);
	NETVAR(m_iObserverMode, "DT_BasePlayer->m_iObserverMode", int);
	NETVAR(aimPunchAngle, "CBasePlayer->m_aimPunchAngle", CVector)
	NETVAR(viewPunchAngle, "CBasePlayer->m_viewPunchAngle", CVector)
...
public: // entity virtual functions
	...
	bool isFlashed() {
		return m_flFlashMaxAlpha() > 250.f;
	}
	...
	constexpr auto getInaccuracy() noexcept {
